      ...
      <div class="span9" id="content">
        <div class="row-fluid">
          ...
          $count_members=mysql_query("SELECT *
            ...
            ?>
            <?php
            $count_member=mysql_query("SELECT *
              FROm  members
              WHERE  DATE_ADD(STR_TO_DATE(Birthday, '%Y-%m-%d'), INTERVAL YEAR(CURDATE())-YEAR(STR_TO_DATE(Birthday, '%Y-%m-%d')) YEAR)
              BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 0 DAY)");
              $countdem = mysql_num_rows($count_member);
              ?>
            ...
            <div id="block_bg" class="block">
              ...
              <div id="block_bg" class="block">
                <div class="navbar navbar-inner block-header">
                  <div class="muted pull-left"><i class="icon-reorder icon-large"></i>Current Birthdays</div>
                  <div class="muted pull-right">
                    Current Birthdays <span class="badge badge-info"><?php  echo $countdem; ?></span>
                  </div>
                </div>
                        ...
                        <table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
                          ...
                          <tbody>
                            ...
                            $members_query = mysql_query("SELECT *
                              FROm  members
                              WHERE  DATE_ADD(STR_TO_DATE(Birthday, '%Y-%m-%d'), INTERVAL YEAR(CURDATE())-YEAR(STR_TO_DATE(Birthday, '%Y-%m-%d')) YEAR)
                              BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 0 DAY)")or die(mysql_error());
                              while($row = mysql_fetch_array($members_query)){
                                $username = $row['id'];
